...
\item \textbf{DP5 Compatibility with Legacy} Avoid disruption of existing 3rd part applications compatibility could represent a major principle to consider. Platform interfaces has to be kept as close as possible to the existing one, preventing application rewrite.
\end{itemize}
...
Nested virtualization refers to a system architecture with two layers of virtualization. It consists in a ``virtualized'' virtualization, where the guest system virtualize a nested guest. The concept could be generalized to an arbitrary number of nested guest layers, introducing recursive virtualization. Virtualization is able to execute tasks in a deprivileged context that normally requires a privileged execution. With nested virtualization we could extend this approach to virtualization itself, executing a guest hypervisor with nested guest VMs, without requiring any change to any of them.
...
Full Virtualization is currently the only virtualization technique supported for L1 virtualization, due to its capacity of running unmodified guest OS and therefore hypervisors. On the contrary, PV would require an ad hoc modified hypervisor to act as L1 (not satisfying NDP1).
